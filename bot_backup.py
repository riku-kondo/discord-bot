...
@app_commands.guilds(
    discord.Object(id=1351599305932275832))  # サーバーIDに置き換えてください
...
async def 挑戦状(interaction: discord.Interaction, 対象ユーザー: str,
    ...
    try:
        ...
        await interaction.followup.send("指定されたユーザーが見つかりませんでした。",
                                        ephemeral=True)
        return
    ...
    if mode_value == "再挑戦":
        embed_all = discord.Embed(
            description=(f"### 🎟️⋆˙⟡{対象メンバー.mention} に**再挑戦**します.𖥔 ݁\n"
                         f"**種目: {種目}**\n"
        ...
        embed_all = discord.Embed(
            description=(f"### 🎟️⋆˙⟡{対象メンバー.mention} に**挑戦状**を送りました.𖥔 ݁\n"
                         f"**種目: {種目}**\n"
    ...
    if mode_value == "再挑戦":
        embed_general = discord.Embed(
            description=(f"### 🎟️⋆˙⟡{実行者.display_name} が **再挑戦** を表明しました.𖥔 ݁\n"
                         f"**種目：{種目}**\n"
                         f"発行日：{date_str}　🤣：{実行者.mention}より"),
            color=discord.Color.orange(),
            timestamp=now)
        ...
        embed_general = discord.Embed(
            description=(f"### 🎟️⋆˙⟡{実行者.display_name} から**挑戦状**が届きました.𖥔 ݁\n"
                         f"**種目：{種目}**\n"
